...
self.requestStatsUpdate = (fieldname) ->
  self.requestStats(fieldname)
...
RiotControl.on 'request_add', ->
  self.update()

self.clearSuccessfulRequests = ->
  self.requests = self.requests.filterByPropertyRange 200, 300

self.clearBadRequests = ->
  self.requests = self.requests.filterByPropertyRange 400, 500
...
self.request_remove = (e) ->
  RiotControl.trigger 'request_remove', e

self.shelve_requests = ->
  RiotControl.trigger 'shelve_requests'

self.unshelve_requests = ->
  RiotControl.trigger 'unshelve_requests'

self.status_toggle =  ->
  self.online_status = if self.online_status == 'online' then 'offline' else 'online'
  self.update()
